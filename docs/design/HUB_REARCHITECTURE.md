...
## 6. 확정 결정 (2026-07-03 — "합치고 단순화")

사용자 승인으로 전 항목 **확정**. 이후 구현은 본 결정을 따른다 (재론 금지).

| # | 결정 | 확정 | 근거 |
|---|---|---|---|
| 1 | repo 구조 | ✅ **모노 (iris-hub 단일 활성 repo)** | 단일사용자 로컬. 분리가 iris-system 이중화의 원인 |
| 2 | DB 구조 | ✅ **단일 SQLite, 계층 테이블 그룹** (볼트+지식) | 개념↔문서 조인이 핵심. 분리는 복잡만 늘림 |
| 3 | 데이터 루트 | ✅ **`~/0Dev/iris-data/{vault,knowledge}`** (hostname 기반) | DB·wiki 한 지붕 → "볼트 어디?" 종결 |
| 4 | 외부 서비스 | ✅ **별도 유지** (OpenWebUI·OpenClaw·Presenton, `integrations/`로 호출) | 컨테이너 독립. 흡수는 과함 |
| 5 | iris-system | ✅ **아카이브** (`iris-legacy-2026`, 삭제 아님), retrieval만 흡수 (S8) | git 히스토리=보험 |
| 6 | 임베딩 | ✅ **FAISS 사이드카** 우선 (nomic-embed 재활용), sqlite-vec는 후속 검토 | 기존 자산 |

### 6.1 이번 사이클 추가 확정 (탭 경계 — 목업 단계에서 정립)

| 경계 | 확정 | 근거 문서 |
|---|---|---|
| 입력 ↔ 흐름 | 입력=수집·인식·1차처리, 흐름=배치적재·K2 | INPUT_INGEST §0 |
| 흐름 ↔ 데이터 | 흐름=명령(실행), 데이터=관측(현황·큐레이션) | DATA_MONITORING §2 |
| 위키 ↔ 그래프 | 위키=개념 정독(근거·출처), 그래프=관계 조망 | WIKI_REBUILD §7 |

### 6.2 REFREEZE 열린 결정 — 전부 해소

| REFREEZE 열린결정 | 해소 |
|---|---|
| 흐름·데이터 1탭 vs 2탭 | **2탭** (명령 vs 관측) — DATA_MONITORING 확정 |
| 위키 검색 먼저 vs 개념 페이지 한번에 | **검색 먼저, 개념 페이지는 S4 후** — WIKI_REBUILD §9 |
| WebUI/OpenClaw 그룹핑 | UI 배치 문제 — UI-UX 단계로 (기능 무영향) |

**→ 남은 열린 결정 0. 구조·경계·데이터 전부 확정. 구현(단계 B) 착수 가능.**
